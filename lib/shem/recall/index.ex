defmodule Shem.Recall.Index do
  @moduledoc """
  ...
  alias Shem.Recall.{Scanner, Text}
  ...
  @doc "Search the corpus. `server` defaults to the supervised singleton."
  ...
  defp refresh(state) do
    ...
    Enum.reduce(current, {%{files: kept}, []}, fn %{session_id: sid, cache_key: key}, {st, skipped} ->
      case st.files do
        %{^sid => %{key: ^key}} ->
          {st, skipped}
  ...
  # never serve it as memory. Legacy (pre-0.5.0) chains still index: old
  # but honest. Runs only when a file changed, so the hashing cost is rare.
